...
extension UserInfoVC: GFRepoItemVCDelegate {
    ...
    func didTapGitHubProfile(for user: User) {
        guard let url = URL(string: user.htmlUrl) else {
             presentAlertOnMainTread(title: "Invalid URL", message: "The url attached to this user is invalid.", buttonTitle: "Ok")
            return
        }
...
extension UserInfoVC: GFFollowerItemVCDelegate {
    ...
    func didTapGetFollowers(for user: User) {
        guard user.followers != 0 else {
            presentAlertOnMainTread(title: "No followers", message: "This user has no followers. What a shame 😔", buttonTitle: "So sad")
            return
        }
